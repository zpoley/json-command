...
JSON.Command.prototype.createObjects = function() {
  ...
  var objects = this.parseObjects();

  console.log("objects in createObjects: " + JSON.stringify(objects));
...
console.log("bojects now: " + JSON.stringify(objects, null, 2));
...
JSON.Command.prototype.processObjects = function(objects) {
  ...
  var preparedObjects = this.prepareFinalObjects(objects);

  try {

    for (var i = 0; (i < (preparedObjects.length)); i++) {

      var po = preparedObjects[i];
...
JSON.Command.prototype.processInput = function() {
  if (this.files) {
    ...
    this.stdin.on("data", function(chunk) {
      ...
      var objects = this.jsonC.createObjects();
